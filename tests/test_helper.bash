setup_clm_env() {
  ...
  mkdir -p "$CLM_ROOT/bin" "$CLM_ROOT/lib/clm/templates" "$CLM_TARGET" "$CLM_DOTFILES_DIR"
  ...
  cp "$BATS_TEST_DIRNAME"/../lib/clm/templates/*.sh "$CLM_ROOT/lib/clm/templates/" 2>/dev/null || true
  chmod +x "$CLM_ROOT/bin/clm" 2>/dev/null || true
  chmod +x "$CLM_ROOT/lib/clm/templates"/*.sh 2>/dev/null || true
}
